        ...
        try {
            ...
            const activitiesToSectionList = activities.map((dayActivity) => ({
                title: {
                    ...
                    dayName: dayjs(dayActivity.date).format("dddd").replace("-feira", ""), //remove o feira da formação padrão
                },
                data: dayActivity.activities.map((activity) => ({
                    id: activity.id,
                    title: activity.title,
                    hour: dayjs(activity.occurs_at).format("hh[:]mm[h]"),
                    isBefore: dayjs(activity.occurs_at).isBefore(dayjs()) //valida se a atividade já passou do horario
                }))
            })
            )
    ...
    return (
        <View className="flex-1">
            <View className="w-full flex-row mt-5 mb-6 items-center">
                <Text
                    className="text-zinc-50 text-2xl font-semibold flex-1">
                    Atividades
                </Text>
